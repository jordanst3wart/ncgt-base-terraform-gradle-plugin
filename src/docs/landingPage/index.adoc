...
The minimum supported Gradle version is `{minimum-gradle-version}`. It has also been tested using https://ysb33rorg.gitlab.io/gradleTest[GradleTest] against a wide range of compatibility. The test matrix for this release in addition to `{minimum-gradle-version}` was
