...
@CompileStatic
class TerraformAwsPlugin implements Plugin<Project> {
    @Override
    void apply(Project project) {
        ...
        terraformSourceSets.all(new Action<TerraformSourceDirectorySet>() {
            ...
            void execute(TerraformSourceDirectorySet tsds) {
                AwsExtension awsExtension = ((ExtensionAware) tsds).extensions.create(
                    AwsExtension.NAME,
                    AwsExtension,
                    project
                )

                tsds.registerCredentialProvider(awsExtension)
            }
        })
    }
}
