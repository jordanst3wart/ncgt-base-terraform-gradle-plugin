...
/**
 * Adds {@link TerraformCloudBackendSpec} backends to the global {@link TerraformBackendExtension} and
 * to each of the same extensions on each {@link TerraformSourceDirectorySet}.
 *
 * Sets a remote state name ({@code key} in terraform speak) on each source set's extension.
 *
 * Tells the global {@link TerraformBackendExtension} to use {@link TerraformCloudBackendSpec} as its backend provider.
 *
 ...
 */
@CompileStatic
class TerraformRemoteStateTerraformCloudPlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
        ...
        TerraformBackendExtension.find(project).addBackend(TerraformCloudBackendSpec.NAME, TerraformCloudBackendSpec)
